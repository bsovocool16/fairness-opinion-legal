...
1. **Board book** (required): the advisor's presentation to the board or committee at the opinion meeting, PDF or PowerPoint. Run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/extract_text.py <file> inputs/book.txt`. If the script reports pages without a text layer, say how many and offer OCR (`--ocr`, needs `ocrmypdf` or `tesseract`) or ask for a text version.
